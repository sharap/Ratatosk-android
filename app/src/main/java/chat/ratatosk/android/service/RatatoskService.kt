...
class RatatoskService : Service() {
    ...
    private fun showIncomingMessageNotification(event: FfiEvent.MessageReceived) {
        ...
        // Use a simpler approach to avoid large context lookup in service if possible
        // but we need a display name. For now let's use the core client to find contact
        val displayName = try {
            if (RatatoskCore.isInitialized()) {
                RatatoskCore.getClient().contacts().find { it.chatId.contentEquals(event.chatId) }?.displayName
            } else null
        } catch (e: Exception) { null } ?: "New Message"
        ...
        try {
            NotificationManagerCompat.from(this).notify(chatIdHex.hashCode(), notification)
